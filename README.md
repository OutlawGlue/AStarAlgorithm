...
I came across **Barricade.gg**, a simple but interesting strategy game based on moving a player across a grid while placing barriers to block the opponent. The idea immediately stood out because the game appears simple on the surface, but it relies heavily on underlying pathfinding logic to determine whether players can still reach their goal.
...
This repository therefore began with the development of a **general-purpose A\*** pathfinding implementation in C#. Now that the algorithm is working reliably, it can be integrated into a larger system that recreates the core mechanics of Barricade-style gameplay.
